<!-- ✨ TotoroKingdom GitHub Profile ✨ -->

<div align="center">

# <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="28"/> 你好，我是 TotoroKingdom

### *用代码编织世界，用 AI 探索未来*

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=18&duration=3000&pause=500&color=7E57C2&center=true&vCenter=true&width=500&lines=Backend+%26+AI+Engineer;LangChain+%2F+LangGraph+Builder;Automation+Enthusiast;%E6%9E%84%E5%BB%BA%E8%87%AA%E5%B7%B1%E7%9A%84%E6%95%B0%E5%AD%97%E4%B8%96%E7%95%8C" alt="Typing SVG" />

</div>

---

## 🧭 关于我

<table>
<tr>
<td width="60%">

- 🔭 正在探索 **AI 工程与后端系统** 的交叉领域  
- 🌱 持续学习 **英语 & AI Agent 系统架构**  
- 🎯 使用 **LLM / LangChain / LangGraph** 构建智能应用  
- 🧩 热爱 **后端开发、系统设计 & AI 产品化**  
- 📫 联系我：**totorokingdom@foxmail.com**

</td>
<td width="40%" align="center">

<img src="[https://media.giphy.com/media/LmNwrBhejkK9EFP504/giphy.gif](https://cdn.pixabay.com/objects3d/2026/05/09/07-36-02-492/render_720_720_0_340_0.png)" width="120" />

</td>
</tr>
</table>

---

## 🧠 技术栈 & 兴趣方向

> *“代码是魔法，而 AI 是新的咒语。”* ✨

<div align="center">

| 领域 | 技术 |
|------|------|
| 🤖 AI 框架 | <img src="https://img.shields.io/badge/LangChain-1C3C3C?style=flat&logo=langchain&logoColor=white"/> <img src="https://img.shields.io/badge/LangGraph-00A86B?style=flat&logo=graph&logoColor=white"/> <img src="https://img.shields.io/badge/RAG-FF6B6B?style=flat&logo=ai&logoColor=white"/> |
| ⚙️ 后端开发 | <img src="https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white"/> <img src="https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white"/> <img src="https://img.shields.io/badge/Java-ED8B00?style=flat&logo=java&logoColor=white"/> |
| 🌐 全栈 | <img src="https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB"/> <img src="https://img.shields.io/badge/Next.js-000000?style=flat&logo=next.js&logoColor=white"/> |
| 🧩 工具 | <img src="https://img.shields.io/badge/Codex-000000?style=flat&logo=openai&logoColor=white"/> <img src="https://img.shields.io/badge/Claude%20Cursor-5A5A5A?style=flat&logo=anthropic&logoColor=white"/> |

</div>

---

## 📊 GitHub 数据

<div align="center">

| <img height="180em" src="https://github-readme-stats-kallkago.vercel.app/api?username=TotoroKingdom&show_icons=true&count_private=true&theme=transparent&hide_border=true"/> | <img height="180em" src="https://github-readme-stats-kallkago.vercel.app/api/top-langs/?username=TotoroKingdom&hide=css,html,scss,less&theme=transparent&hide_border=true&layout=compact"/> |
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|

</div>

---

## 🎮 原神 · 专区

<div align="center">

[![genshin-image]][genshin-link]

*“大丘丘病了，二丘丘瞧～”* 🍡

</div>

---

## 💬 趣闻小站

<div align="center">

|  |  |
|:---:|:---:|
| 🌌 | “这个人其实一直在偷偷构建自己的世界” |
| 🍵 | “压力大的时候，泡杯茶，写点代码就好了” |
| 🎵 | “写代码时的 BGM 比代码本身更重要” |

</div>

---

## ⚡ 当前专注

<div align="center">

| 🧠 | 🛠 | 📈 | 🚀 |
|:---:|:---:|:---:|:---:|
| 深化 AI Agent 架构理解 | 构建AI作品集项目 | 强化英语沟通能力 | 探索 AI 自动化产品落地 |

</div>

---

<div align="center">

<img src="https://komarev.com/ghpvc/?username=TotoroKingdom&color=7E57C2&style=flat&label=✨+来访者" alt="visitor badge"/>

**感谢你的停留，愿派蒙与你同在 ✨**

</div>

<!-- ======================== 链接 ======================== -->

[genshin-image]: https://genshin-card-kallka.vercel.app/rand/310652688.png
[genshin-link]: https://enka.network/u/217189114
